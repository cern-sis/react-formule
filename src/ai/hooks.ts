...
export const useCleanFieldTypes = () => {
  const { allFieldTypes } = useContext(CustomizationContext);

  return useMemo(() => cleanFieldTypes(allFieldTypes), [allFieldTypes]);
};
...
export const useGetProvider = (): string | null => {
  ...
  return useMemo(() => getProvider(providers), [providers]);
};
